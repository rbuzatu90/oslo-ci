...
foreach ($projectNameGit in $projectsList)
{
    if ($projectNameGit -ne $projectName)
    {
        if ( $projectNameGit -ne "os-win" -or @("stable/mitaka", "master") -contains $branchName.ToLower()) {
            ExecRetry {
                GitClonePull "$buildDir\$projectNameGit" "https://git.openstack.org/openstack/$projectNameGit.git" $branchName
            }
        }
    }
}
...
foreach ($projectNameInstall in $projectsList)
{
    ExecRetry {
        if ($isDebug -eq  'yes') {
            Write-Host "Content of $buildDir\$projectNameInstall"
            Get-ChildItem $buildDir\$projectNameInstall
        }
        pushd $buildDir\$projectNameInstall
        & pip install -e $buildDir\$projectNameInstall
